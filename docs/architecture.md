...
SCIP Certificate Lifecycle has two workflows: certificate issuance and expiry monitoring.
...
2. `PRODUCT_TEAM` and `ENVIRONMENT` select `scip/cert-lifecycle/certs/${PRODUCT_TEAM}-${ENVIRONMENT}-certs.yml`.
3. `APP_NAME` selects one app, or an empty value processes all apps sequentially.
4. Jenkins calls `issueCertificate(app)` from `scip-platform-lib`.
5. The helper validates the app, derives `/scip/certs/{app.name}`, and assumes `jagent-ec2-role` in the spoke account.
...
5. It reads `/scip/certs/{app.name}` and extracts only the `certificate` field.
...
| --- | --- |
| `> 30` | log only |
| `15..30` | publish to `scip-cert-renewal` |
| `<= 14` | publish to `scip-cert-p1-alerts` |
...
| --- | --- | --- | --- |
| `jagent-ec2-role` | Spoke | Jenkins issuance | create/update/tag `/scip/certs/*` |
| `CertLifecycleRole` | Spoke | Lambda expiry checker | read `/scip/certs/*` |
| `scip-cert-expiry-checker-role` | Shared services | Lambda runtime | read Bitbucket token, assume spoke roles, publish SNS, write logs |
